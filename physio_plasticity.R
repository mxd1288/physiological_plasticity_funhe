...
ggplot(wam_diff, aes(x=tenth_perc_MO2_mg_hr.x, y=tenth_perc_MO2_mg_hr.y, col=accl_order)) + geom_point() + geom_smooth(method=lm) + 
  labs(x="12°C Metabolic Rate", y="28°C Metabolic Rate") + 
  theme_bw()
...
#####
# Figure 2
wam_12_plasticity <- ggplot(wam_diff, aes(x=tenth_perc_MO2_mg_hr.x, y=log2_ratio_accl)) + geom_point(size=2, aes(col=plas_group, shape=accl_order.x)) + 
...
wam_28_plasticity <- ggplot(wam_diff, aes(x=tenth_perc_MO2_mg_hr.y, y=log2_ratio_accl)) + geom_point(size=2, aes(col=plas_group, shape=accl_order.x)) + 
  geom_smooth(method=lm, color="black", alpha=1) + 
...
paneld <- ggplot(wam_diff, aes(x=tenth_perc_MO2_mg_hr.x, y=tenth_perc_MO2_mg_hr.y, col=plas_group)) + geom_point(size=2, aes(shape=accl_order.x)) + 
  labs(x="12°C Metabolic Rate", y="28°C Metabolic Rate") + 
  theme_bw() +
...
#ggsave("wam_plas_fig_revised.eps", wam_plas_fig, width=10, height=8)
#######################################################################
...
ctmax_12_plasticity_2 <- ggplot(ctmax_diff2, aes(x=ctmax.x, y=ctmax_log2ratio)) + geom_point(size=2.5,aes(shape=accl_order.x)) + 
  geom_smooth(method=lm, alpha=1) + 
  labs(x="12°C CTmax", y="Plasticity in CTmax (log2 ratio)") + 
  annotate("text", x=37, y = 0.32, label="p < 0.001***, R2 = 0.93") + 
  theme_bw()
ctmax_12_plasticity_2
# p < 2e-16 ***, R2= 0.9012
...
ctmax_28_plasticity2 <- ggplot(ctmax_diff2, aes(x=ctmax.y, y=ctmax_log2ratio)) + geom_point(size=2.5, aes(shape=accl_order.x)) + 
  geom_smooth(method=lm, alpha=1) + 
  labs(x="28°C CTmax", y="Plasticity in CTmax") + 
  annotate("text", x=42.8, y = 0.4, label="p = 0.71, R2 = -0.009") + 
  theme_bw()
ctmax_28_plasticity2

# p =  0.9293, R2 = 9.194e-05
summary(lm(ctmax_diff$ctmax_log2ratio~ctmax_diff$ctmax.y))

# log2_ratio versus ctmax 12°C by acclimation order group
fits_ctmax <- lmList(ctmax_log2ratio ~ ctmax.x | accl_order.x, data=ctmax_diff)

summary(fits_ctmax)$r.squared

#ggsave("ctmax_12_plasticity_revised.eps", ctmax_12_plasticity_2)
#ggsave("ctmax_28_plasticity_revised.eps", ctmax_28_plasticity2)
...
# CV of plasticity for WAM
wam_diff2 <- subset(wam_diff, wam_diff$log2_ratio_accl!="NA")

# CV in WAM plasticity
mean(wam_diff2$log2_ratio_accl)
sd(wam_diff2$log2_ratio_accl)

(0.6328684/1.068153)*100

tapply(wam_diff2$log2_ratio_accl, wam_diff2$accl_order.x, mean)

tapply(wam_diff2$log2_ratio_accl, wam_diff2$accl_order.x, sd)

# WAM accl group 1 CV in plasticity == 66.67%
(0.6007603/0.9011007)*100

# WAM accl group 2 CV in plasticity == 50.99%
(0.6297857/1.2352052)*100
...
# CV of plasticity within an acclimation group
ctmax_diff2 <- subset(ctmax_diff, ctmax_diff$ctmax_log2ratio!="NA")

# CV in CTMax plasticity
mean(ctmax_diff2$ctmax_log2ratio)
sd(ctmax_diff2$ctmax_log2ratio)

# 13.75617
(0.03174164/0.2307448)*100

tapply(ctmax_diff2$ctmax_log2ratio, ctmax_diff2$accl_order.x, mean)

tapply(ctmax_diff2$ctmax_log2ratio, ctmax_diff2$accl_order.x, sd)

# accl group 1 CV in plastcity == 9.69%1
(0.02375874/0.2451833)*100

# accl group 2 CV in plastcity == 13.35%
(0.02727188 /0.2041966 )*100
